...
from common_conf.xpath.remote_management import *
    ...
    while repeat_times < 4:
        ...
        try:
            ...
            if wwa_status_class == expect_status:
                print('web wan access 是勾选状态')
            else:
                print('web wan access 是未勾选状态')
                sleep(3)
...
def check_web_wan_access(driver,expect_operation_type,expect_status=None,expect_port=None,expect_ip=None):
    ...
    while repeat_times < 4:
        ...
        if operation_type == expect_operation_type and message_status == 'Succeeded':
            ...
            return False
        try:
            print(f'这是第{repeat_times}次检查web wan access页面')
            if expect_status is not None:
                ...
                print("期望的状态：" + expect_status)
                if actual_status != expect_status:
                    if repeat_times==3:
                        print('期望的状态与实际的状态不一致')
                        return False
                    assert False
            if expect_port is not None:
                ...
                print("实际的端口号：" + actual_port)
                print("期望的端口号：" + expect_port)
                if actual_port != expect_port:
                    if repeat_times==3:
                        print('期望的端口号与实际的端口号不一致')
                        return False
                    assert False
            if expect_ip is not None:
                #获取web wan access的ip
                ...
                print("实际的ip：" + actual_ip)
                print("期望的ip：" + expect_ip)
                if actual_ip != expect_ip:
                    if repeat_times==3:
                        print('期望的ip与实际的ip不一致')
                        return False
                    assert False
            return True
        except:
