...
from common_conf.xpath.vpn import *
from common_fun.user_public_fun import move_to_message_center
...
def add_vpn(driver,except_name,except_server,except_username,except_protocol,except_password=None,except_mppe=None,except_mppc=None):
    #添加vpn
...
def check_vpn(driver,except_name,expect_operation_type,except_server=None,except_username=None,except_protocol=None,
    ...
    while repeat_times < 4:
        ...
        if operation_type == expect_operation_type and message_status == 'Succeeded':
            ...
            return False
        try:
            print(f'这是第{repeat_times}次检查VPN配置')
            WebDriverWait(driver, 10).until(
            ...
            if except_name is not None:
                ...
                print('预期的名称为', except_name)
                print('实际的名称为', actual_name)
                if actual_name != except_name:
                    if repeat_times == 3:
                        print('预期的名称和实际的名称不一致，配置失败')
                        return False
                    assert False
            if except_server is not None:
                ...
                print('预期的服务器为', except_server)
                print('实际的服务器为', actual_server)
                if actual_server != except_server:
                    if repeat_times == 3:
                        print('预期的服务器和实际的服务器不一致，配置失败')
                        return False
                    assert False
            if except_username is not None:
                ...
                print('预期的用户名为', except_username)
                print('实际的用户名为', actual_username)
                if actual_username != except_username:
                    if repeat_times == 3:
                        print('预期的用户名和实际的用户名不一致，配置失败')
                        return False
                    assert False
            if except_protocol is not None:
                ...
                print('预期的协议为', except_protocol)
                print('实际的协议为', actual_protocol)
                if actual_protocol != except_protocol:
                    if repeat_times == 3:
                        print('预期的协议和实际的协议不一致，配置失败')
                        return False
                    assert False
...
def delet_vpn(driver,except_name):
    ...
    repeat_times = 1
    while repeat_times < 4:
        print(f'这是第{repeat_times}次删除VPN')
        try:
            WebDriverWait(driver, 10).until(
...
def check_delet_vpn(driver,except_name,expect_operation_type):
    ...
    while repeat_times < 4:
        ...
        if operation_type == expect_operation_type and message_status == 'Succeeded':
            ...
            return False
        try:
            print(f'这是第{repeat_times}次检查删除是否成功')
