import unittest
from common_conf.conf import TCP_UDP_Port_forwarding, TCP_Port_forwarding, \
    UDP_Port_forwarding, edit_Port_forwarding,delete_Port_forwarding
from common_fun import port_forwarding_fun
from common_fun.base import Base
class Port_Forwarding(Base):

    def test_1_add_tcp_udp_port_forwarding(self):
        """新增一条协议为”TCP&UDP“的端口转发[NOC-8365]"""
        port_forwarding_fun.add_port_forwarding(self.driver,expect_portname=TCP_UDP_Port_forwarding["name"],#端口名
        ...
        result=port_forwarding_fun.check_port_forwarding(self.driver,expect_ip=TCP_UDP_Port_forwarding["local_ip"],
                                    ...
                                    expect_Protocol=TCP_UDP_Port_forwarding["Protocol_TCP/UDP"]) # TCP/UDP协议
        if result:
            print('添加TCP&UDP协议的端口转发成功')
            assert True
        else:
            print('添加TCP&UDP协议的端口转发失败')
            assert False

    def test_2_add_tcp_port_forwarding(self):
        """新增一条协议为”TCP“的端口转发[NOC-8366]"""
        port_forwarding_fun.add_port_forwarding(self.driver,expect_portname=TCP_Port_forwarding["name"],#端口名
        ...
        result=port_forwarding_fun.check_port_forwarding(self.driver,expect_ip=TCP_Port_forwarding["local_ip"],
                                    ...
                                    expect_Protocol=TCP_Port_forwarding["Protocol_TCP"]) # TCP协议
        if result:
            print('添加TCP协议的端口转发成功')
            assert True
        else:
            print('添加TCP协议的端口转发失败')
            assert False

    def test_3_add_udp_port_forwarding(self):
        """新增一条协议为”UDP“的端口转发[NOC-8367]"""
        port_forwarding_fun.add_port_forwarding(self.driver, expect_portname=UDP_Port_forwarding["name"],  # 端口名
        ...
        result = port_forwarding_fun.check_port_forwarding(self.driver, expect_ip=UDP_Port_forwarding["local_ip"],
                                   ...
                                   expect_Protocol=UDP_Port_forwarding["Protocol_UDP"])  # UDP协议
        if result:
            print('添加UDP协议的端口转发成功')
            assert True
        else:
            print('添加UDP协议的端口转发失败')
            assert False

    def test_4_edit_port_forwarding(self):
        """修改所有的信息能修改成功[NOC-8329]"""
        port_forwarding_fun.edit_port_forwarding(self.driver,upd_ip=UDP_Port_forwarding['local_ip'], # UPD协议的本地ip
        ...
        result=port_forwarding_fun.check_port_forwarding(self.driver,expect_ip=edit_Port_forwarding["local_ip"],
                                    ...
                                    expect_Protocol=edit_Port_forwarding["Protocol_TCP/UDP"]) # TCP/UDP协议
        if result:
            print('修改UDP协议的端口转发成功')
            assert True
        else:
            print('修改UDP协议的端口转发失败')
            assert False

    def test_5_delete_port_forwarding(self):
        """端口转发：列表删除按钮，点击有toast提示[NOC-8386]"""
        port_forwarding_fun.delete_port_forwarding(self.driver,expect_ip=delete_Port_forwarding["local_ip"] # TCP协议的本地ip
        ...
        result=port_forwarding_fun.check_delet_port_forwarding(self.driver,expect_ip=delete_Port_forwarding["local_ip"],# TCP协议的本地ip
                                                        expect_portnum3=delete_Port_forwarding["port_num3"],# 第一个本地端口号
                                                        expect_portnum4=delete_Port_forwarding["port_num4"],# 第二个本地端口号
                                                        expect_operation_type=delete_Port_forwarding["type"])# 操作类型
        if result:
            print('删除TCP协议的端口转发成功')
            assert True
        else:
            print('删除TCP协议的端口转发失败')
            assert False
